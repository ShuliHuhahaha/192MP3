...
```{r}
western.new <- read.csv('/Users/hushuli/Desktop/senior/SDS192/MP3/western.csv')
```
...
```{r}
western.movie <- db %>%
  dbGetQuery("SELECT m.movie_id, t.title, t.production_year, cn.name, ct.kind, country_code
...
western.movie1 <- western.movie %>%
  mutate(company_name = name, company_kind = kind, movie_title=title) %>%
  select(-name, -kind, -title)

company10 <- western.movie1 %>%
  group_by(company_name) %>%
  summarize(count = n()) %>% 
  arrange(desc(count)) %>%
  head(10)

company_country <- western.movie1 %>%
  filter(company_kind== "production companies") %>%
...
```{r}
#compare the amount of western film produced by US companies and non-US companies
western_foreign <- western.movie1 %>%
  filter(country_code != "[us]") %>%
  filter(company_kind== "production companies") %>%
  select(movie_id, production_year) %>%
  group_by(production_year) %>%
  summarise(amount_foreign = n_distinct(movie_id)) %>%
  arrange(production_year)

western_us <- western.movie1 %>%
  filter(country_code == "[us]") %>%
  filter(company_kind== "production companies") %>%
  select(movie_id, production_year) %>%
  group_by(production_year) %>%
...
western_foreign_us <- western_foreign %>%
  full_join(western_us) 


ggplot(western_foreign_us, aes(x=production_year)) +
  geom_line(aes(y=amount_foreign, col="amount_foreign")) +
  geom_line(aes(y=amount, col="amount"))
