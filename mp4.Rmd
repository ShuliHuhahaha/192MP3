...
ggplot(western.sum, aes(x = production_year)) +
  ...
  annotate("text", x = 1912, y = 410, label = "1912", size = 4, color = "red") +
...
movie.genre.2014 <- movie.genre %>%
  ...
  arrange(desc(number_movie))

datatable(movie.genre.2014,
          colnames = c('Genre', '# Movie'),
          width = 600,
           options = list(columnDefs = list(list(
            className = 'dt-center', targets = 0
          ))),
          caption = htmltools::tags$caption(
    style = 'text-align: center; font-size:150% ;',
    'Movie Genre Production in 2014')
          )
```
